...
import { formatDate, formatTime, formatPhone, getStatusBadgeClasses } from '../../lib/format-utils';
import { supabase } from '../../../services/supabaseClient'; // Ensure you have a Supabase client setup
...
function DashboardContent() {
  ...
  const fetchDashboardData = useCallback(async () => {
    setLoading(true);
    try {
      // Use the admin service to fetch all dashboard data in one go
      const dashboardData = await adminService.getDashboardStats();
      
      // Update the stats and recent reservations
      ...
      try {
        ...
        // Check if there are new orders since last refresh
        if (previousOrdersCount > 0 && ordersData.length > previousOrdersCount) {
          const newCount = ordersData.length - previousOrdersCount;
          ...
          setTimeout(() => setShowNewOrdersAlert(false), 10000);
        }
      } catch (orderErr) {
  ...
  // Set up auto-refresh to check for new orders every 30 seconds
  useEffect(() => {
    if (authLoading || !user) return;
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      setLoading(true);
      try {
        // Use the server-side API endpoint to fetch users securely
  ...
  return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 min-h-screen transition-all duration-300">
        ...
        {/* New Orders Alert */}
        {showNewOrdersAlert && (
          <div className="mb-8 bg-gradient-to-r from-green-50 to-green-100 dark:from-green-900/30 dark:to-green-800/20 p-5 rounded-2xl shadow-xl backdrop-blur-sm animate-fadeIn border border-green-200 dark:border-green-800/50">
            <div className="flex">
              ...
              <div className="ml-4 my-auto">
                <p className="text-base font-medium text-green-700 dark:text-green-200">
                  {newOrdersCount} new {newOrdersCount === 1 ? 'order has' : 'orders have'} been placed!
                </p>
              </div>
              <button 
                onClick={() => setShowNewOrdersAlert(false)}
                className="ml-auto bg-green-200 dark:bg-green-800 rounded-full p-1 hover:bg-green-300 dark:hover:bg-green-700 transition-colors duration-200"
              >
                <svg className="h-5 w-5 text-green-500 dark:text-green-200" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                </svg>
              </button>
            </div>
                  ...
                  <div className="space-y-4">
                    {recentReservations.map((reservation) => (                      <div key={reservation.reservation_id || reservation.id} className="p-4 bg-gradient-to-r from-gray-50/50 to-white/50 dark:from-gray-800/30 dark:to-gray-800/40 rounded-xl hover:bg-white/70 dark:hover:bg-gray-700/50 transition-all duration-300 backdrop-blur-sm border border-gray-100/30 dark:border-gray-700/30 hover:border-gray-200/50 dark:hover:border-gray-600/50 shadow-sm hover:shadow-md group">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                          ...
                          <a 
                            href={`/admin/reservations?id=${reservation.reservation_id || reservation.id}`} 
            ...
            <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-gray-100/50 dark:border-gray-700/50 hover:shadow-2xl transition-all duration-300">
              ...
              <div className="p-6">
                <nav className="space-y-4">
                  ...
                  {/* Add Orders Quick Action */}
                  <a 
                    href="/admin/orders" 
                    className="flex items-center p-4 rounded-2xl bg-gradient-to-r from-gray-50/70 to-white/70 dark:from-gray-800/40 dark:to-gray-800/60 shadow hover:shadow-lg text-gray-700 dark:text-gray-200 transition-all duration-300 border border-gray-100/50 dark:border-gray-700/50 hover:border-amber-200 dark:hover:border-amber-900/50 transform hover:-translate-y-0.5 group"
                  >
                    <div className="p-3 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-600 text-white shadow-lg group-hover:shadow-xl transition-all duration-300">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                      </svg>
                    </div>
                    <div className="ml-4">
                      <p className="font-bold text-lg text-gray-900 dark:text-white">Manage Orders</p>
                      <p className="text-sm font-medium text-gray-600 dark:text-gray-300">View and update customer orders</p>
                    </div>
                  </a>
        ...
        <div className="mt-10">
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-gray-100/50 dark:border-gray-700/50 transition-all duration-300 hover:shadow-2xl">
            <div className="px-6 py-5 border-b border-gray-200/50 dark:border-gray-700/50 bg-gradient-to-r from-gray-50/80 to-white/80 dark:from-gray-800/80 dark:to-gray-800/80">
              <h3 className="text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 dark:from-indigo-400 dark:via-purple-400 dark:to-pink-300">Registered Users</h3>
            </div>
            <div className="p-6">
              {loading ? (
                <p>Loading users...</p>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {users.map((user) => (
                        <tr key={user.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{user.user_metadata?.name || 'N/A'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div id="orders-section" className="mt-10">
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-gray-100/50 dark:border-gray-700/50 transition-all duration-300 hover:shadow-2xl">
            <div className="px-6 py-5 border-b border-gray-200/50 dark:border-gray-700/50 bg-gradient-to-r from-gray-50/80 to-white/80 dark:from-gray-800/80 dark:to-gray-800/80">
              <h3 className="text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 dark:from-amber-400 dark:via-orange-400 dark:to-red-300">Recent Orders</h3>
            </div>
            <div className="p-6">
              {loading ? (
                <p>Loading orders...</p>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {orders.map((order, index) => (
                        <tr key={order.id || index} className={order.status === 'pending' ? 'bg-amber-50 dark:bg-amber-900/20' : ''}>
